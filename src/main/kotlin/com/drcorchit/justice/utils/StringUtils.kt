...
object StringUtils {
	...
	fun capitalizeWord(input: String, always: Boolean): String {
		return when (input) {
			"a", "and", "the" -> {
				...
				if (!always) input else input.substring(0, 1)
					.uppercase(Locale.getDefault()) + input.substring(1)
					.lowercase(Locale.getDefault())
			}
	...
	fun substring(input: String?, size: Int): String {
		return if (input == null) "null" else if (input.length <= size) input else input.substring(
			0,
			size
		)
	}
